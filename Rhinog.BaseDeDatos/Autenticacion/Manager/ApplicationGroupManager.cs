...
namespace Rhinog.BaseDeDatos
{
    public class ApplicationGroupManager
    {
        ...
        public async Task<IdentityResult> AgregarGruposYRoles(string UsuarioId, List<ApplicationGroup> GruposYRoles)
        {

            //Remuevo  los grupos del usuario que tiene actualmente
            var GruposActuales = await this.GetUserGroupsAsync(UsuarioId);
            foreach (var grupo in GruposActuales)
            {
                grupo.ApplicationUsers.Remove(grupo.ApplicationUsers.FirstOrDefault(gr => gr.ApplicationUserId == UsuarioId));
            }
            await _db.SaveChangesAsync();

            foreach (var grupo in GruposYRoles)
            {
                if (grupo.EstaSeleccionado)
                {
                    var nuevoGrupo = await this.FindByIdAsync(grupo.Id);
                    nuevoGrupo.ApplicationUsers.Add(new ApplicationUserGroup { ApplicationUserId = UsuarioId, ApplicationGroupId = grupo.Id });

                    if (grupo.RolGuardar)
                        await this._userManager.AddToRoleAsync(UsuarioId, "Guardar");
                    if (grupo.RolActualizar)
                        await this._userManager.AddToRoleAsync(UsuarioId, "Actualizar");
                    if (grupo.RolEliminar)
                        await this._userManager.AddToRoleAsync(UsuarioId, "Eliminar");
                    if (grupo.RolConsultar)
                        await this._userManager.AddToRoleAsync(UsuarioId, "Consultar");
                    if (grupo.RolImprimir)
                        await this._userManager.AddToRoleAsync(UsuarioId, "Imprimir");
                    if (grupo.RolExportar)
                        await this._userManager.AddToRoleAsync(UsuarioId, "Exportar");
                }
            }
            await _db.SaveChangesAsync();

            await this.RefreshUserGroupRolesAsync(UsuarioId);
